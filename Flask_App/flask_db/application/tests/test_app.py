...
class TestBase(TestCase):
    ...
    class TestHome(TestBase):
        def test_home_get(self):
            ...
            self.assertIn(b'load home page', response.data)
    ...
    class TestAddTruffle(TestBase):
        def test_addTruffle_get(self):
            ...
            self.assertEqual(response.status_code, 200)
            self.assertIn(b'add_truffle', response.data)
    ...
    class TestUpdateTruffle(TestBase):
        def test_update_truffle(self):
            response = self.client.post(
                url_for('update', title="Artisanal truffle grancherie 30g"),
                data = dict(title = "Artisanal truffle grancherie"),
    ...
    class TestDel(TestBase):
        def test_delete_truffle(self):
            response = self.client.get(url_for('delete', title = "Artisanal truffle grancherie 30g"), follow_redirects=True)
            self.assertEqual(response.status_code, 200)
